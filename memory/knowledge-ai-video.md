...
```
想法 → 剧本 → 美术（场景/角色/道具）→ 分镜（镜头运动+内容）→ 视频素材
         ↑
    P工有美术背景，可以自己出概念设计
```
...
| 环节 | 工具 | 作用 |
|------|------|------|
| 概念图 | Midjourney | 美术概念，风格探索 |
| 三视图 | 即梦（香蕉AI）| 角色/场景多角度 |
| 多角度图 | 即梦（香蕉AI）| 分镜关键帧素材 |
| 手绘构图 | 人工手绘 | 角色+场景融合，构图控制 |
| 底图全面化 | Midjourney + 即梦 | 光影/色调/气氛在底图阶段做全 |
| 视频生成 | 即梦 / 小云雀 / 海螺 | 最终视频输出 |
| 剪辑 | 剪映 | 素材剪辑合成 |
| 音乐/音效 | 后期制作 | 后期合成 |

### 9.3 核心原则：底图定生死

> **出视频最关键的是底图。在底图阶段就要做到全面（光影、色调、气氛），这样后续才能更好控制整体画面。**

```
底图不全面 → 视频阶段无法修复 → 只能重抽 → 成本暴增
底图做好 → 后续可控 → 少量抽卡 → 成本可控
```
...
> 几秒的镜头有时花2-3000积分，因为不知道AI在哪一块会崩。

**优化策略：**
- 底图做充分的前期设计（美术判断力是核心壁垒）
- 关键帧多抽优选
- 视频生成时固定参考图，减少随机性
- 保留足够积分预算用于后期抽卡
